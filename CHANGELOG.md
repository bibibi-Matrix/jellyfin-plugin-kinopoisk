...
- Хронометраж фильма (Runtime)
- Бэкдропы из галереи КиноПоиска
- Метаданные эпизодов сериалов (названия, описания, даты)
- Ограничение частоты запросов (rate limiting), настраивается
- Настраиваемый таймаут запросов
- Использован IHttpClientFactory для проверки URL картинок
